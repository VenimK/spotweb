...
class Services_NzbHandler_Factory
{
    public static function build(Services_Settings_Container $settings, $action, array $nzbHandling)
    {
        ...
        /*
         * Keep queue-capable handlers even when the client is temporarily offline,
         * so the UI can still show status / connection errors (NZBGet, SABnzbd, …).
         * Only fall back to Disable for non-API handlers that fail availability.
         */
        if (!$handler instanceof Services_NzbHandler_Disable && $handler->isAvailable() !== true) {
            $api = $handler->hasApiSupport();
            $hasQueueApi = is_string($api) && $api !== '' && strpos($api, 'getStatus') !== false;
            if (!$hasQueueApi) {
