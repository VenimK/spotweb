...
abstract class Services_NzbHandler_abs
{
    ...
    {
        /*
         * Prefer a root-relative URL so downloads work even when spotweburl
         * is still the installer placeholder (or points at another host).
         * Browsers resolve this against the current origin.
         */
        $action = $this->_nzbHandling['action'];
        $url = '?page=getnzb&amp;action='.$action.'&amp;messageid='.urlencode($spot['messageid']).$spotwebApiParam;
