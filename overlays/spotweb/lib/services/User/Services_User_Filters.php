...
class Services_User_Filters
{
    ...
    public function filtersToXml($filterList)
    {
        ...
        foreach ($filterList as $filter) {
             ...
             * and excludes
             */
            $dynaList = explode(',', (string) ($filter['tree'] ?? ''));
            list($categoryList, $strongNotList) = $svcSearchQP->prepareCategorySelection($dynaList);
            $treeList = explode(',', (string) ($svcSearchQP->compressCategorySelection($categoryList, $strongNotList) ?? ''));
             ...
             */
            $tmpFilterValues = explode('&', (string) ($filter['valuelist'] ?? ''));
            $filterValueList = [];
