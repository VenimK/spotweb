...
class Dao_Base_Cache implements Dao_Cache
{
    ...
    public function expireCache($expireDays)
    {
         ...
         */
        /*
         * ttl = 0 means "keep until retention says otherwise".
         * Without that guard, (0 + stamp) < now matches every row and
         * a retention cleanup would wipe the whole cache every run.
         */
        $expiredList = $this->_conn->arrayQuery(
            'SELECT id, resourceid, cachetype, metadata FROM cache WHERE stamp < :stamp1 OR (ttl > 0 AND (ttl + stamp) < :stamp2)',
            [':stamp1'    => [time() - ($expireDays * 24 * 60 * 60), PDO::PARAM_INT],
                ':stamp2' => [time(), PDO::PARAM_INT],
            ]
        );
    ...
    protected function isCached($resourceid, $cachetype)
    {
        $tmpResult = $this->_conn->arrayQuery(
            'SELECT 1 FROM cache WHERE resourceid = :resourceid AND cachetype = :cachetype AND (ttl + stamp) < :expirestamp',
            [':resourceid'     => [$resourceid, PDO::PARAM_STR],
                ':cachetype'   => [$cachetype, PDO::PARAM_INT],
                ':expirestamp' => [time(), PDO::PARAM_INT],
            ]
        );
    ...
    protected function oldCalculateFilePath($cacheId, $cacheType, $metadata)
    {
         ...
         * Get the cache id
         */
        $cacheRow = $this->_conn->singleQuery(
            'SELECT resourceid FROM cache WHERE id = :cacheid',
            [
                ':cacheid' => [$cacheId, PDO::PARAM_INT],
            ]
        );
    ...
    protected function saveCache($resourceid, $cachetype, $metadata, $ttl, $content)
    {
        ...
        $this->_conn->exec(
            'UPDATE cache SET stamp = :stamp, metadata = :metadata, ttl = :ttl WHERE resourceid = :resourceid AND cachetype = :cachetype',
            [
                ':stamp'      => [time(), PDO::PARAM_INT],
                ':metadata'   => [$serializedMetadata, PDO::PARAM_STR],
                ':ttl'        => [$ttl, PDO::PARAM_INT],
                ':resourceid' => [$resourceid, PDO::PARAM_STR],
                ':cachetype'  => [$cachetype, PDO::PARAM_INT],
            ]
        );
        ...
        if ($this->_conn->rows() == 0) {
            $this->_conn->modify(
                'INSERT INTO cache(resourceid,cachetype,stamp,ttl,metadata) VALUES (:resourceid, :cachetype, :stamp, :ttl, :metadata)',
                [
                    ':resourceid' => [$resourceid, PDO::PARAM_STR],
                    ':cachetype'  => [$cachetype, PDO::PARAM_INT],
                    ':stamp'      => [time(), PDO::PARAM_INT],
         ...
         */
        $cacheRow = $this->_conn->singleQuery(
            'SELECT id FROM cache WHERE resourceid = :resourceid AND cachetype = :cachetype',
            [
                ':resourceid' => [$resourceid, PDO::PARAM_STR],
                ':cachetype'  => [$cachetype, PDO::PARAM_INT],
            ]
        ...
        if (!$this->putCacheContent($cacheRow, $cachetype, $content, $metadata)) {
             ...
             */
            $this->_conn->exec(
                'DELETE FROM cache WHERE resourceid = :resourceid AND cachetype = :cachetype',
                [
                    ':resourceid' => [$resourceid, PDO::PARAM_STR],
                    ':cachetype'  => [$cachetype, PDO::PARAM_INT],
                ]
    ...
    protected function updateCacheStamp($resourceid, $cachetype)
    {
         ...
         */
        $this->_conn->exec(
            'UPDATE cache SET stamp = :stamp WHERE resourceid = :resourceid AND cachetype = :cachetype AND ttl = 0',
            [
                ':stamp'      => [time(), PDO::PARAM_INT],
                ':resourceid' => [$resourceid, PDO::PARAM_STR],
                ':cachetype'  => [$cachetype, PDO::PARAM_INT],
    ...
    /*
     * Intentionally do not refresh NZB cache stamps on access.
     *
     * Valid NZBs are stored with ttl=0. Refreshing stamp on every download
     * prevented retention/expire-cache from deleting them, so cache/nzb grew
     * without bound (multi‑GB). Stamp stays at first-cache time so expireCache()
     * can remove NZBs older than retention days.
     */
    ...
    * Retrieve a translated comment resource from the cache
     ...
     */
    public function saveTranslatedCommentCache($resourceId, $language, $translations)
    {
        return $this->saveCache(
            $resourceId.'_'.$language,
