...
// Multi-NZB checkboxes live in table cells (td.multinzb) and card views (.spotCard .multi)
function multiNzbCheckboxes(checkedOnly) {
	var sel = 'td.multinzb input[type="checkbox"], .spotCard .multi input[type="checkbox"], #checkboxget input[name="messageid[]"]';
	var $boxes = $(sel);
	return checkedOnly ? $boxes.filter(':checked') : $boxes;
}

function multinzb() {
	var count = multiNzbCheckboxes(true).length;
	if(count == 0) {
...
function checkMultiNZB() {
	var $active = $("tr.active td.multinzb input[type=checkbox], .spotCard.sw-focus .multi input[type=checkbox], .spotCard.sw-focus input[name='messageid[]']");
	if(!$active.length) {
		$active = $("tr.sw-focus td.multinzb input[type=checkbox]");
	}
	if(!$active.length) {
		return;
	}
...
function toggleAllMultiNzb() {
    var $master = $('th.multinzb input[type=checkbox]');
    var val = $master.is(':checked');
    multiNzbCheckboxes(false).prop('checked', val);
    multinzb();
} // toggleAllMultinzb


function downloadMultiNZB(dltype) {
	var $checked = multiNzbCheckboxes(true);
	...
	if(count > 0) {
		...
		$checked.each(function() {
			url += '&messageid%5B%5D='+encodeURIComponent($(this).val());
		});
        ...
        if (dltype != 'display') {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    multiNzbCheckboxes(false).prop("checked", false);
                    multinzb();
                    if (data.result == 'failure') {
                        alert('Error occured\n'+data.errors[0]);
                    }
