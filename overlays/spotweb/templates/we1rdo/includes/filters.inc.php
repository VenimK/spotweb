...
<?php
    ...
    foreach ($parsedsearch['filterValueList'] as $filterType) {
        if (in_array($filterType['fieldname'], ['Titel', 'Title', 'Poster', 'Tag', 'SpotterID'])) {
...
?>
						...
						<input type="hidden" name="sortdir" value="<?php if ($sortType == 'stamp' || $sortType == 'spotrating' || $sortType == 'commentcount') {
    echo 'DESC';
} else {
    echo 'ASC';
} ?>">
...
<?php if (!isset($ageFilter)) {
    $ageFilter = '';
						...
						<ul class="search clearCategories onecol">
							...
							<label><?php if ($parsedsearch['unfiltered'] == 'true') {
    echo _('Use categories');
} else {
    echo _("Don't use categories");
} ?></label> </li>
						</ul>
...
<?php } ?>
						<a href="<?php echo htmlspecialchars($tplHelper->makeFilterManagerUrl(), ENT_QUOTES, 'UTF-8'); ?>" class="greyButton filterManager" target="_blank" rel="noopener" title="<?php echo htmlspecialchars(_('Open the filter manager'), ENT_QUOTES, 'UTF-8'); ?>"><?php echo _('Filter manager'); ?></a>
				</div>
			</form>
